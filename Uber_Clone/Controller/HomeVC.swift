...
class HomeVC: UIViewController, Alertable {
    ...
    func connectUserToDriver(){
        ...
        DataService.instance.passengerIsOnTrip(passangerKey:Auth.auth().currentUser?.uid, handler: { (isOnTrip, driverKey, tripKey) in
                if isOnTrip == true {
    ...
    func actionSelector(forAction action: buttonAction){
        switch action {
        case .requestRide:
            if !destinationTxt.text!.isEmpty{
        ...
        case .directiontoDestination:
            DataService.instance.driverIsOnAtrip(driverKey: Auth.auth().currentUser?.uid, handler: { (isOnTrip, driverKey, tripKey) in
                if isOnTrip == true {
                    DataService.instance.REF_TRIPS.child(tripKey!).child("DestinationCoordinates").observe(.value, with: { (snapshot) in
    ...
    func performSearch(){
        ...
        search.start { (response, error) in
            if error != nil {
                ...
                self.destinationTxt.text = ""
                self.destinationTxt.resignFirstResponder()
